...
'''
NIKHDREfex.py
call Nik's Collection's HDR Efex Pro 2 passing the active layer as a temp file. 
...
def plugin_main( image, drawable, visible ):
  ...
  if visible == 0:
    ...
    temp = pdb.gimp_layer_new_from_visible(image, image, "NIKHDR")
  ...
  command = "\"C:\\Program Files\\Google\\Nik Collection\\HDR Efex Pro 2\\HDR Efex Pro 2 (64-Bit)\\HDR Efex Pro 2.exe\"" + " \"" + tempfilename + "\""
  args = shlex.split(command)

  # Invoke external command
  pdb.gimp_progress_set_text ("calling HDR Pro 2...")
  ...
  try:
	hdrfile = os.path.expanduser( "~/Documents/ShellOutTempFile_HDR.tif")
	gimp_log( "The HDR file is " + hdrfile )
	copyfile( hdrfile, tempfilename )
  ...
  os.remove( hdrfile )     # delete the locally created HDR file
...
register(
        "python_fu_HDR_efex_pro",
        "Call HDR Efex Pro 2",
        "Call HDR Efex Pro 2",
        "Rob Antonishen",
        "Copyright 2011 Rob Antonishen",
        "2011",
        "<Image>/Photography/HDR/Nik HDR Efex Pro 2",
        "RGB*, GRAY*", 
