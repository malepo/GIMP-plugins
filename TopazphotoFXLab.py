...
'''
Dfine2.py
call Topaz photoFXlab passing the active layer as a temp file. 
...
def plugin_main( image, drawable, visible ):
  ...
  if visible == 0:
    ...
    temp = pdb.gimp_layer_new_from_visible(image, image, "TopazPhotoFXLab")
  ...
  command = "\"c:\\Program Files (x86)\\Topaz Labs\\Applications\\64Bit\\photoFXlab.exe\"" + " \"" + tempfilename + "\""
  args = shlex.split(command)

  # Invoke external command
  pdb.gimp_progress_set_text ("calling photoFXlab...")
...
register(
        "python_fu_Topaz_photoFXlab",
        "Call Topaz photoFXlab",
        "Call Topaz photoFXlab",
        "Rob Antonishen",
        "Copyright 2011 Rob Antonishen",
        "2011",
        "<Image>/Photography/Topaz Photo FXlab",
        "RGB*, GRAY*", 
